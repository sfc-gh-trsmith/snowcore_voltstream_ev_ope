...
def create_agent_via_rest(
    ...
    Args:
        ...
        replace: If True, use PUT to replace existing agent. If False, use POST to create.
    ...
    if replace:
        # PUT to update existing agent (agent name in URL, not required in payload)
        url = f"https://{host}/api/v2/databases/{database}/schemas/{schema}/agents/{agent_name}"
        print(f"Calling REST API: PUT {url}", file=sys.stderr)
        print(f"Agent name: {agent_name} (replacing existing)", file=sys.stderr)
        ...
        # POST to create new agent (agent name must be in payload)
        url = f"https://{host}/api/v2/databases/{database}/schemas/{schema}/agents"
        
        # Ensure the agent name is in the payload
        if "name" not in create_body:
            create_body["name"] = agent_name
        
        print(f"Calling REST API: POST {url}", file=sys.stderr)
        print(f"Agent name in payload: {create_body.get('name')}", file=sys.stderr)
