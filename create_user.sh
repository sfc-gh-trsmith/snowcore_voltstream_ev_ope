...
# Priority: 1) Schema matching PROJECT_PREFIX, 2) EV_OPE (data mart), 3) *_MART schema, 4) First non-system schema
if echo "$DISCOVERED_SCHEMAS" | grep -qw "$DEFAULT_PROJECT_PREFIX"; then
    DEFAULT_SCHEMA="$DEFAULT_PROJECT_PREFIX"
elif echo "$DISCOVERED_SCHEMAS" | grep -qw "EV_OPE"; then
    # EV_OPE is the standard data mart schema for this project
    DEFAULT_SCHEMA="EV_OPE"
elif echo "$DISCOVERED_SCHEMAS" | grep -qE "_MART$"; then
    # Look for any schema ending in _MART
